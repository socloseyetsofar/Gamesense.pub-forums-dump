...
//$links[] = '<li id="navsupport"><a class="fake-link" id="support" href="invite.php"><i class="fa fa-lg fa-ticket" id="support-badge" data-after-content="0"></i></a></li>';
...
// New PMS
	...
	<?php }
	$mytopid = $pun_user['id'];
	$result447 = $db->query("SELECT * FROM `gs_codes` WHERE `by`='$mytopid' AND `used`='0'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
   // $invby = $db->fetch_assoc($result447);
	if (!$pun_user['is_guest'] && basename($_SERVER['PHP_SELF']) == "index.php" && $db->num_rows($result447) && $pun_user['g_id'] != 4){
	?>	<div class='notice-bar'><a href='profile.php?section=premium&id=<?php echo $mytopid ?>'>You have unused invitation codes!</a></div>
	<?php }
	$version_loader = $db->query("SELECT version FROM `gs_versions` WHERE `id`='1'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
    $version_loader = $db->fetch_assoc($version_loader);
	if($pun_user['g_id'] != 4 && $pun_user['g_id'] != 3 && $version_loader == 'NULL')
	{
?>	<div class='notice-bar'><a href='profile.php?section=premium&id=<?php echo $mytopid ?>'>New client is available!</a></div>
	<?php }
