...
if (isset($_POST['form_sent']))
{
	...
	if($csgoupdates != null)
	$db->query("UPDATE `gs_updates` SET `updated` = '$csgoupdates' WHERE `id` = '1'") or error('Unable to update', __FILE__, __LINE__, $db->error());
	
	$csgobetaupdates = pun_trim($_POST['form']['csgo_beta']);
	if($csgobetaupdates != null)
	$db->query("UPDATE `gs_updates` SET `updated` = '$csgobetaupdates' WHERE `id` = '2'") or error('Unable to update', __FILE__, __LINE__, $db->error());


	$csgoversion = pun_trim($_POST['form']['csgo_version']);
	if($csgoversion != null)
	$db->query("UPDATE `gs_versions` SET `version` = '$csgoversion' WHERE `id` = '2'") or error('Unable to update', __FILE__, __LINE__, $db->error());
	
	
	$loaderversion = pun_trim($_POST['form']['loader_version']);
	if($loaderversion != null)
	$db->query("UPDATE `gs_versions` SET `version` = '$loaderversion' WHERE `id` = '1'") or error('Unable to update', __FILE__, __LINE__, $db->error());
...
?>
 ...
 $updates_csgo = $db->query("SELECT * FROM `gs_updates` WHERE `id`='1'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $updates_csgo = $db->fetch_assoc($updates_csgo);   
	 $updates_csgo_beta = $db->query("SELECT * FROM `gs_updates` WHERE `id`='2'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 ...
 $version_csgo = $db->query("SELECT * FROM `gs_versions` WHERE `id`='2'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $version_csgo = $db->fetch_assoc($version_csgo);   
						
						?>
							<table class="aligntop" cellspacing="0">
								<tr>
									<th scope="row">csgo</th>
									<td>
