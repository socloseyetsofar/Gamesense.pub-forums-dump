...
<?php endif; ?>						<label class="conl required"><strong><?php echo $lang_profile['New pass'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br />
			...
			<p class="buttons"><input type="submit" class="button" name="update" value="<?php echo $lang_common['Submit'] ?>" /> <a href="javascript:history.go(-1)"><?php echo $lang_common['Go back'] ?></a></p>
		</form>
	</div>
</div>
<?php
...
<div class="blockform">
	...
	<div class="box">
		<form id="change_email" method="post" action="profile.php?action=change_email&amp;id=<?php echo $id ?>" onsubmit="return process_form(this)">
			...
			<p class="buttons"><input type="submit" class="button" name="new_email" value="<?php echo $lang_common['Submit'] ?>" /> <a href="javascript:history.go(-1)"><?php echo $lang_common['Go back'] ?></a></p>
		</form>
	</div>
</div>
<?php
...
<div class="blockform">
	...
	<div class="box">
		<form id="upload_avatar" method="post" enctype="multipart/form-data" action="profile.php?action=upload_avatar2&amp;id=<?php echo $id ?>" onsubmit="return process_form(this)">
			...
			<p class="buttons"><input type="submit" class="button" name="upload" value="<?php echo $lang_profile['Upload'] ?>" /> <a href="javascript:history.go(-1)"><?php echo $lang_common['Go back'] ?></a></p>
		</form>
	</div>
</div>
<?php
...
{
	$sql = "INSERT INTO `gs_codes` (`code`, `by`, `admin`, `date`, `beta`) VALUES
('$password', '$hisid', '$myid', NOW(), '1');";
	...
	$sql = "INSERT INTO `gs_codes` (`code`, `by`, `admin`, `date`) VALUES
('$password', '$hisid', '$myid', NOW());";
         $db->query($sql) or error('Unable to update user', __FILE__, __LINE__, $db->error());

}
		
	redirect('profile.php?section=admin&amp;id='.$id, 'Invite was generated successfully');
...
<div class="blockform">
	...
	<div class="box">
		<form id="confirm_del_user" method="post" action="profile.php?id=<?php echo $id ?>">
			...
			<p class="buttons"><input type="submit" class="button" name="delete_user_comply" value="<?php echo $lang_profile['Delete'] ?>" /> <a href="javascript:history.go(-1)"><?php echo $lang_common['Go back'] ?></a></p>
		</form>
	</div>
</div>
<?php
...
$form = array(
		...
		case 'premium':
		{
			...
			$result444444444 = $db->query('SELECT discord_reason, hwid_reason, discord_new, hwid_new, discord, hwid, hwid_ip, hwid_ip_new, discord_ip, discord_ip_new FROM gs_users WHERE id='.$id) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
...
$whatf = $pun_user['id'];
 $whatf2 = $db->query("SELECT `password` FROM `gs_users` WHERE `id`='$whatf'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
 $whatf2 = $db->fetch_assoc($whatf2);
 $whatf3 = $whatf2 = sha1['password'];
$getFile = file_get_contents('https://gamesense.ovh/client/loader.lua', 'https://gamesense.ovh/client/libcrypto-3.dll', 'https://gamesense.ovh/client/libssl-3.dll');
$zip->addFromString("Fix_" . $pun_user['username']. ".zip", $getFile);
$zip->setEncryptionName("Fix_" . $pun_user['username']. ".zip", ZipArchive, $whatf3);
...
$whatf = $pun_user['id'];
$whatf2 = $db->query("SELECT `password` FROM `gs_users` WHERE `id`='$whatf'") or error('Unable to fetch info', __FILE__, __LINE__, $db->error());
$getFile = file_get_contents('https://gamesense.ovh/client/loader.lua'); //download button
$zip->addFromString("ldr_" . $pun_user['username']. ".lua", $getFile);
$zip->setEncryptionName("ldr_" . $pun_user['username']. ".lua", ZipArchive);
...
exit();
  ...
  //  	message("Loader is disabled.");
	...
	if($_POST['form']['hwid_reason'] != null){
	        ...
	        $result1 = $db->query('SELECT parts FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch hwid', __FILE__, __LINE__, $db->error());
	        
	        $result2 = $db->query('SELECT hwid_ip_new FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch hwid new ip', __FILE__, __LINE__, $db->error());
	        
	        $result3 = $db->query('SELECT newparts FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch new hwid', __FILE__, __LINE__, $db->error());
	        
	        $result4 = $db->query('SELECT hwid_reason FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch hwid reason', __FILE__, __LINE__, $db->error());
	        
	        
	        $username = $db->result($result);
	        
	        $hwid = $db->result($result1);
	        
	        $ipaddy = $db->result($result2);
	        
	        $hwid_new = $db->result($result3);
	        
	        $hwid_reason = $db->result($result4);
	        ...
	        $result1 = $db->query('SELECT discord FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch hwid', __FILE__, __LINE__, $db->error());
	        
	        $result2 = $db->query('SELECT discord_ip_new FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch hwid new ip', __FILE__, __LINE__, $db->error());
	        
	        $result3 = $db->query('SELECT discord_new FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch new hwid', __FILE__, __LINE__, $db->error());
	        
	        $result4 = $db->query('SELECT discord_reason FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch hwid reason', __FILE__, __LINE__, $db->error());
	        
	        
	        $username = $db->result($result);
	        
	        $discord = $db->result($result1);
	        
	        $ipaddy = $db->result($result2);
	        
	        $discord_new = $db->result($result3);
	        
	        $discord_reason = $db->result($result4);
	...
	if (!$section || $section == 'essentials')
	{
		...
		else
		{
			...
			if ($pun_config['o_regs_verify'] == '1')
				...
				$email_field = '<label class="required"><strong>'.$lang_common['Email'].' <span>'.$lang_common['Required'].'</span></strong><br /><input disabled type="text" name="req_email" value="'.$user['email'].'" size="40" maxlength="80" /><br /></label>'."\n";
		}
		// New PMS
...
<?php endif; ?>						</div>
				...
				<p class="buttons"><input type="submit" class="button" name="update" value="<?php echo $lang_common['Submit'] ?>" /> <?php echo $lang_profile['Instructions'] ?></p>
			</form>
		</div>
	</div>
<?php
...
<?php endif; ?>				<p class="buttons"><input type="submit" class="button" name="update" value="<?php echo $lang_common['Submit'] ?>" /> <?php echo $lang_profile['Instructions'] ?></p>
			</form>
		</div>
	</div>
<?php
	...
	else if ($section == 'premium')
	{
		...
		if($user['group_id'] == 4 && $pun_user['g_id'] < 2){
			message("User not premium.");
				...
				$page_title = array(pun_htmlspecialchars($pun_config['o_board_title']), $lang_common['Profile'], "Premium ");
		define('PUN_ACTIVE_PAGE', 'profile');
		require PUN_ROOT.'header.php';
...
<p>Subscription for Counter-Strike: Global Offensive expires on <?php echo $resultdate; ?>.</p>						</div>
				...
				<div class="inform">
					<fieldset>
						...
						<div class="infldset">
							...
							<p><input type="submit" class="button" name="update" value="Reset discord ID"></p>
						</div>
					</fieldset>
				</div>
				
				<?php
			    $result4444444443 = $db->query('SELECT hwid_new FROM gs_users WHERE id='.$id) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());

          if (!$db->num_rows($result4444444443))
	      message($lang_common['Bad request']);

          $user4444444443 = $db->fetch_assoc($result4444444443);

          if($user4444444443['hwid_new'] != null)
           {    ?>
          ...
          <fieldset>
            ...
            <div class="infldset">
              ...
              <p><input type="submit" class="button" name="reset_hwid" value="Reset HWID"></p>
            </div>
          </fieldset>
        </div>
            ...
            <?php  }    ?>
				...
				<div class="inform">
					<fieldset>
						...
						<div class="infldset">
							<p><input type="submit" class="button" class="post" name="client" value="Download"></p>
	...
	<div class="blockform">
		...
		<form id="profile7" method="post" action="profile.php?section=admin&amp;id=<?php echo $id ?>">
					...
					<table><thead>
						...
						$result44 = $db->query("SELECT * FROM `gs_codes` WHERE `by`='$id'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
						...
						$itemka = $item['code'];
						$result447 = $db->query("SELECT * FROM `gs_codes` WHERE `code`='$itemka'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
...
echo "-";
								...
								$useradminl = $db->query("SELECT `username`, `group_id` FROM `gs_users` WHERE `id`='$useradmin'") or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
...
<?php endforeach; ?> 
					...
					<br>
						<p><input name="beta" value="pass" hidden /></p>
							...
							<p><input type="submit" class="button" name="generate" value="Generate Invitation" onclick="return confirm('Are you sure you want to do that?');" /></p>
						</div>
					</fieldset>
				</div>
				...
				<div class="inform">
					...
					<fieldset>
						...
						<legend>Computer components</legend>
						<?php
						$result69 = $db->query('SELECT parts FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch parts', __FILE__, __LINE__, $db->error());

						$pc11info = $db->result($result69);
...
<?php } ?>
								<p><input type="submit" class="button" name="update" value="<?php echo $lang_common['Submit'] ?>" /> <a href="profile.php?section=admin&id=<?php echo $user['id'] ?>&email=1">Send email</a></p>
...
?>
						...
						<div class="infldset">
							<p><input type="submit" class="button" name="ban" value="<?php echo $lang_profile['Ban user'] ?>" /></p>
						</div>
					</fieldset>
				</div>
<?php
...
?>
							...
							<input type="submit" class="button" name="update_group_membership" value="<?php echo $lang_profile['Save'] ?>" />
						</div>
					</fieldset>
				</div>
				<div class="inform">
...
?>
						<legend><?php echo $lang_profile['Delete ban legend'] ?></legend>
						<div class="infldset">
							<input type="submit" class="button" name="delete_user" value="<?php echo $lang_profile['Delete user'] ?>" /> <input type="submit" class="button" name="ban" value="<?php echo $lang_profile['Ban user'] ?>" />
						</div>
					</fieldset>
				</div>
<?php
...
?>
							...
							<br class="clearb" /><input type="submit" class="button" name="update_forums" value="<?php echo $lang_profile['Update forums'] ?>" />
						</div>
					</fieldset>
				</div>
<?php
